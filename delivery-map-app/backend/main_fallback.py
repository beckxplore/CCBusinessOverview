...
def get_clickhouse_client():
    """Create ClickHouse client with fallback configuration."""
    try:
        import clickhouse_connect
        
        # Try to get from environment first
        host = os.getenv('CLICKHOUSE_HOST', 'localhost')
        port = int(os.getenv('CLICKHOUSE_PORT_STR', '8123'))
        username = os.getenv('CLICKHOUSE_USER', 'default')
        password = os.getenv('CLICKHOUSE_PASSWORD', '')
        database = os.getenv('CLICKHOUSE_DATABASE', 'default')
        secure = os.getenv('CLICKHOUSE_SECURE_STR', 'false').lower() == 'true'
        verify = os.getenv('CLICKHOUSE_VERIFY_STR', 'false').lower() == 'true'
